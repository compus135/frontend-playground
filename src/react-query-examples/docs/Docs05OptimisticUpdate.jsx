...
import doRequest from "./data/docs05";
...
async function fetchTodos() {
  const res = await doRequest({ method: "GET" });
  return res;
}
...
function Example() {
  ...
  const addTodoMutation = useMutation({
    mutationFn: (newTodo) =>
      doRequest({ method: "POST", body: { text: newTodo } }),
  ...
  return (
    <div>
      ...
      {addTodoMutation.error && addTodoMutation.error.message}
    </div>
  );
}
